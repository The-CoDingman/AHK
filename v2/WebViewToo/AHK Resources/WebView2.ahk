		...
		if (createdEnvironment)
			...
			if (!FileExist(dllPath) && FileExist(t := A_LineFile '\..\' (A_PtrSize * 8) 'bit\WebView2Loader.dll')) {
				if (!A_IsCompiled) {
					...
					MsgBox("About to try and load DLL from internal resource")
					Module := DllCall("GetModuleHandle", "Ptr", 0, "Ptr")
					Resource := DllCall("FindResource", "Ptr", Module, "Str", "64bit\WebView2Loader.dll", "UInt", RT_RCDATA := 10, "Ptr")
					ResourceSize := DllCall("SizeofResource", "Ptr", Module, "Ptr", Resource)
					ResourceData := DllCall("LoadResource", "Ptr", Module, "Ptr", Resource, "Ptr")
					MsgBox("ResourceSize: " ResourceSize)
					;dllPath := ResourceData
					dllPath := "moooo"
					;dllPath := t
				}
			}
